#!/usr/bin/env python3
"""Implementation for patched Games arcade rendering validation."""
...
def collect_network_errors(
    browser_logs: Sequence[dict[str, Any]],
    performance_logs: Sequence[dict[str, Any]],
    origin: str,
) -> list[Any]:
    errors: list[Any] = []
...
def run(repo: pathlib.Path, site: pathlib.Path, lessons: pathlib.Path, output: pathlib.Path) -> int:
    ...
    try:
        ...
        for mode in ("desktop", "mobile-reduced"):
            ...
            try:
                ...
                # The website contract is explicit: gCard() creates
                # <h4><span></span>...</h4> and then writes g.title into that
                # first span. Read the same node the production renderer writes.
                titles = driver.execute_script("""
                  return [...document.querySelectorAll('#allGrid .gcard h4 > span:first-child')]
                ...
                result["errors"].extend(collect_network_errors(
                    driver.get_log("browser"), driver.get_log("performance"), server.origin
                ))
                if count != EXPECTED_COUNT:
                    result["errors"].append(f"expected {EXPECTED_COUNT} cards, rendered {count}")
                if len(titles) != count:
                    result["errors"].append(f"expected one title node per card; found {len(titles)} titles for {count} cards")
    ...
    (output / "arcade-browser-results.json").write_text(
    ...
    if failures:
        ...
        lines.append(
            "Desktop and mobile/touch rendering both passed with one title node per card, "
            "one visible title-based release holder and no legacy description marker."
        )
