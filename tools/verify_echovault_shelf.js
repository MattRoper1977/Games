#!/usr/bin/env node
/*
 * verify_echovault_shelf.js — shelf contract for the Echo Vault manifest entry.
 *
 ...
 *
 *   node tools/verify_echovault_shelf.js
 */
'use strict';
const fs = require('fs');
...
pass(matches.length === 1, `Echo Vault appears exactly once (found ${matches.length})`);
...
pass(/echo\s*vault/i.test(game.title), 'title names the game');
...
pass(!('collection' in game),
  'no rail membership claimed — Echo Vault is a general-shelf entry, not an Apex Sports one');
...
pass(/sound/i.test(desc), 'card copy leads with the sonar identity: sound is your light');
pass(/danger|hunter/i.test(desc), 'card copy names the danger sound brings');
...
/* ---- positive controls: each family must be shown able to fail ---- */
{
  ...
  const ok = [
    ...
    control('card copy stripped of its identity', c => { c.games.find(g => g.href === HREF).desc = 'A game.'; },
      c => { const d = c.games.find(x => x.href === HREF).desc; if (!/sound/i.test(d) || !/danger|hunter/i.test(d)) throw new Error('x'); })
  ];
  pass(ok.every(Boolean), `positive controls all tripped their gate (${controls.join('; ')})`);
}

function report() {
  console.log(`\nEcho Vault shelf contract: ${checks - failures.length}/${checks} checks passed.`);
